...
\begin{large} Matrix models play an important role in Physics ranging from nuclear 
physics where they were first introduced 
to study of random surfaces, conformal field theories, 
integrable systems, two-dimensional quantum gravity, 
and non-perturbative descriptions of M-theory. In these notes, 
we consider a wide variety of matrix models and study them 
...
and expectations for some unsolved models are provided
to assist future bootstrapping. In order to encourage this exchange, 
we provide codes in \PY for convenience of the reader and explain how 
...
The codes we present were tested on a laptop and took
...
%Consider a potential $V(M)$, 
%\begin{equation}
%V(M) = -\frac{y^2}{2} + \frac{gy^4}{4} + g^2 \zeta  %\frac{y^6}{6} 
%\end{equation}
...
various numerical computations. This method was introduced in 1987 by Duane, Kennedy, Pendleton, and Roweth \cite{Duane:1987de} who put together the ideas from
...
and molecular dynamics (MD) methods. For a detailed review about HMC and its extension to rational HMC which is required for fermions, the interested readers can consult the Refs. \cite{Hanada:2018fnp,Joseph:2019zer}. The two basic parts of HMC are: 
...
\begin{figure}[htbp] 
	...
	\caption{\label{fig:RN}In the limit of large sample size (here 1 million), it tends to Gaussian with mean zero and $\sigma=1$. For exammple, if this was not sure the the MC results would often converge to the incorrect answer.}
\end{figure}
...
The leapfrog method is used to numerically integrate differential equations. This is a second-order method and the energy non-conservation depends on square of step-size used. This integrator is `symplectic', i.e., it preserves the 
...
\begin{figure}[htbp] 
	...
	Right: The parabola-like distribution for $g = 0, h = -0.04965775$ with $N=300$.}
\end{figure}
...
This corresponds to breaking the $\mathbb{Z}_{2}^{\otimes 3}$ symmetry and just keeping the $X \to Y$ part. 
...
%We obtain for $g=2, c=\ kappa=1.35$, $t_{2} = 0.741(2)$ and $t_{4} = 0.825(3)
%We obtain for $g=1, c=\kappa=1.35$, $t_{2} = 1.577(2)$ and $t_{4} = 3.093(3)$
...
The full discussion of these models is outside the purpose of this article. 
}
...
In the original model, since it is a ten-dimensional reduction we have, $\mu, \nu = 1 \cdots 10$. It is opossible to consider other models where $\mu, \nu = 1 \cdots D$ where $D=4,6$. One might worry whether because of the integral measure being over non-compact $X$, it is divergent. The convergence issues of the partition function of 
...
One of the observables (also known as `size' or i.e., extent of scalars) which we compute in these models is:
...
%(which is real and bounded i.e., where the MC methods can be applied), 
