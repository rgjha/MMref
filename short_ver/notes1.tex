...
to the study of random surfaces, conformal field theories, 
...
we provide programs in \PY~for convenience of the reader and explain how 
...
between few minutes to several hours to finish depending on the model
...
\end{equation}
it is straightforward to show that the odd moments of X i.e., $ t_{n} = (1/N)\mbox{Tr} X^n = 0$ for odd $n$
...
Another interesting dimer matrix model closely related to one matrix model was 
studied in context of understanding the Yang-Lee edge singularity
...
models which have a dual gravity interpretation or emergent geometry behaviour. 
...
by running for 1.7 hours on the laptop. The results are with $g=h=1$ and $N=300$. For larger $N$ say, $N=800$, 
...
\end{equation}
This serves as a good check of the code and is satisfied to an accuracy of 
0.02-0.03\% after ignoring thermalization cut. 
We studied this model using MC for $D=3,5$ with $h=1, \lambda=4$ and compute: 
...
$t_{4}$ change. It is an interesting problem (in practice, not in principle) 
to understand how one can apply bootstrap methods away from the planar limit where 
factorization does not hold any more.  
\begin{mdframed}[backgroundcolor=blue!3] 
	\textsc{} 
