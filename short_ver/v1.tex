...
Gross-Witten-Wadia (GWW) model \cite{Gross:1980he,Wadia:2012fr}
...
We use Monte Carlo methods to study the open and closed cases of the model with $p = 3,4$ with $N = 300$. It would be interesting if this four matrix model can be \textit{bootstrapped} in the coming years. The $p$-matrices MC code to solve these models (well-tested and currently with for $p = 3,4$) is available at: 
...
highly efficient parallelized code (over $N$) for only BFSS and BMN models is available at
...
\end{mdframed}
This is an exact result and must be satisfied during the simulation. We show in Fig.~\ref{fig:YM_allD} that for 
