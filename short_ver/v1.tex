% v1: November 2021
% v1: Contains 1 Mathematica code, 3 Python codes, 5 tables, 10 exercises, 14 figures, 52 references, 1+51 pages. Codes available at https://github.com/rgjha/MMMC
...
The plan of this work is to introduce the numerical solution of matrix models in the large $N$ limit using Monte Carlo (MC) methods and implement them to solve several multi-matrix models where no other analytical/numerical treatment is yet possible. The motivation for this work is partly from the recent progress in the numerical bootstrap program to solve matrix models and we hope that this introduction with the \PY~codes will assist those explorations and serve as a cross-check of the future results. Matrices have played an important role in Physics for a long time. Their presence is ubiquitous in many different areas ranging from nuclear physics to the study of random surfaces, 
...
no one knows about such an operator! We refer the reader to the excellent books \cite{Meh2004, Akemann:2011csh} for introductions to the field of random matrix theory. 
...
The saddle-point analysis is useful to solve matrix models in the planar limit, but, it is not very useful to study $1/N$ corrections. The preferred method for this is that of orthogonal polynomials (OP). This was introduced by Bessis et al. in Ref.~\cite{Bessis:1980ss} and can be used to study matrix models with one and more matrices. 
...
\end{equation}
In order to illustrate how we can solve matrix models using this method, we use OP to study the Ising model on random graph in Appendix~\ref{sec:Ortho_pol1}. One can also use this method to study unitary matrix models like the Gross-Witten-Wadia (GWW) model \cite{Gross:1980he,Wadia:2012fr}
as was done in Ref.~\cite{Goldschmidt:1979hq}. In addition to this model, there are other unitary models which are interesting for lattice gauge theory in lower dimensions. 
One such model is the external field problem which was considered in \cite{Brezin:1980rk} and consists of a model of unitary links in external source field. It was solved with a fixed external field in the large $N$ limit and found to have a third-order phase transition. This model reduces to the GWW model when the external source is set to unit matrix.  These developments in large $N$ $\rm{QCD}_{2}$ models also lead (after a few years) to the idea of volume reduction in large $N$ limit known as Eguchi-Kawai reduction \cite{Eguchi:1982nm} in which it was shown that in the planar limit, the lattice gauge theory for an infinite lattice and unit cube are identical. The space-time seems incorporated in the large $N$ limit as an internal degree of freedom. There are some subtle requirements for this idea to work and are technical but this has led to lot of interesting work, see for instance Ref.~\cite{Kovtun:2007py}. 
...
\end{equation} 
where $N$ is the size of the matrix and $V$ is the potential of the model. The basic steps of the leapfrog algorithm which is time-interleaved is given below: 
...
We mentioned in Sec.~\ref{subsec:OP} that the method of polynomials can be used to solve the Ising model on random graph. We sketch the solution for the interested reader. One can show that the partition function written in the usual form: 
\begin{equation}
...
quantum mechanics model. But, in few months, this disappeared. I would like to thank Simon Catterall for this exercise.} but this is not true. 
...
We give a sample code for computing statistical errors of output data files in \PY~and refer the reader to Ref.~\cite{2012arXiv1210.3781Y} for more details. The program can be used from a terminal as follows: 
